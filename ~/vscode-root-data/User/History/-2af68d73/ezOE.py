...
# Nova query DQL (Data Query Language) que aceita um parâmetro
SQL_GET_BUYS_BY_TICKER = f"""
SET search_path TO {SCHEMA_NAME}, public;
SELECT 
    a.ticker,
    t.quantity,
...
JOIN 
    transaction t ON a.asset_id = t.asset_id
WHERE 
    t.transaction_type = 'BUY' AND a.ticker = %s;
"""

def get_buy_transactions_by_ticker(ticker: str) -> pd.DataFrame:
    """
    Busca transações de COMPRA para um Ticker específico.
    Usa consultas parametrizadas (%s) para PREVENIR SQL INJECTION.
    """
    ...
    try:
        # 1. A query usa %s como placeholder.
        # 2. O 'params=(ticker,)' passa o valor de forma segura.
        df = pd.read_sql(SQL_GET_BUYS_BY_TICKER, conn, params=(ticker,))
        
        if df.empty:
            print(f"⚠️ Repositório: Nenhuma transação 'BUY' encontrada para o ticker {ticker}.")
        else:
            print(f"✅ Dados extraídos para o ticker {ticker}.")
            
        return df

    except Exception as e:
        print(f"❌ ERRO no Repositório: Falha ao buscar dados por ticker. Detalhes: {e}")
        return pd.DataFrame()
    finally:
        if conn:
            conn.close()
