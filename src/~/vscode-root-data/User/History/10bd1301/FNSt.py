...
def insert_initial_data(conn):
   ...
   # 1. SQL for inserting a User (Remove o prefixo do schema)
    SQL_INSERT_USER = f"""
    ...
    # 2. SQL for inserting sample Assets (Remove o prefixo do schema)
    SQL_INSERT_ASSETS = f"""
    ...
    # 3. SQL for inserting Mock Transactions (Fully Quoted and Qualified)
    ...
    try:
        with conn.cursor() as cur:
            # Execute the schema path command BEFORE the inserts
            cur.execute(SQL_SET_SCHEMA) 
            
            # Execute User Insertion (Assumes a fresh database or handling conflicts)
            ...
            # Execute Asset Insertion
            cur.execute(SQL_INSERT_ASSETS)

            # Execute Mock Transactions Insertion
